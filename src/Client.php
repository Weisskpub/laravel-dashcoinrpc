...
class Client
{
    ...
    public function __construct($config = [])
    {
        ...
        $handlerStack->push(
            Middleware::mapResponse(function (ResponseInterface $response) {
                return DashcoindResponse::createFrom($response);
            }),
            'json_response'
        );
    ...
    /**
     * Makes request to dashcoin Core.
     *
     * @param string $method
     * @param mixed  $params
     *
    ...
    public function request($method, $params = [])
    {
        try {
            ...
            if ($response->hasError()) {
                ...
                throw new Exceptions\DashcoindException($response->error());
            }
            ...
            if (
                ...
                throw new Exceptions\DashcoindException($exception->getResponse()->error());
            }
        ...
        } catch (Exceptions\DashcoindException $exception) {
            throw $exception;
        }
    }

    /**
     * Makes async request to dashcoin Core.
     *
     * @param string        $method
     * @param mixed         $params
        ...
        callable $onRejected = null)
    {
        ...
        $promise->then(
            function (ResponseInterface $response) use ($onFullfiled) {
                $error = null;
                if ($response->hasError()) {
                    $error = new Exceptions\DashcoindException($response->error());
                }
            ...
            function (RequestException $exception) use ($onRejected) {
                if (
                    ...
                    $exception = new Exceptions\DashcoindException(
    ...
    /**
     * Converts amount from satoshi to dashcoin.
     *
     * @param int $amount
     *
     * @return float
    ...
    /**
     * Converts amount from dashcoin to satoshi.
     *
     * @param float $amount
     *
     * @return int
